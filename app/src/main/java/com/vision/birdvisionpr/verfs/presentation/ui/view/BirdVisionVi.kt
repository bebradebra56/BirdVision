...
class BirdVisionVi(
    private val birdVisionContext: Context,
    private val birdVisionCallback: BirdVisionCallBack,
    private val birdVisionWindow: Window
) : WebView(birdVisionContext) {
    private var birdVisionFileChooserHandler: ((ValueCallback<Array<Uri>>?) -> Unit)? = null
    fun birdVisionSetFileChooserHandler(handler: (ValueCallback<Array<Uri>>?) -> Unit) {
        this.birdVisionFileChooserHandler = handler
    }
    ...
    init {
        ...
        webSettings.apply {
            ...
            userAgentString =
                WebSettings.getDefaultUserAgent(birdVisionContext).replace("; wv)", "")
                    .replace("Version/4.0 ", "")
        ...
        super.setWebViewClient(object : WebViewClient() {
            override fun shouldOverrideUrlLoading(
                ...
                return if (request?.isRedirect == true) {
                    ...
                    try {
                        birdVisionContext.startActivity(intent)
                    } catch (e: Exception) {
                        Toast.makeText(
                            birdVisionContext,
            ...
            override fun onPageFinished(view: WebView?, url: String?) {
                ...
                if (url?.contains("ninecasino") == true) {
                    BirdVisionApplication.birdVisionInputMode =
                        WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN
                    Log.d(
                        BirdVisionApplication.BIRD_VISION_MAIN_TAG,
                        "onPageFinished : ${BirdVisionApplication.birdVisionInputMode}"
                    )
                    birdVisionWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_PAN);
                    birdVisionEnableStableInputScroll()
                } else {
                    BirdVisionApplication.birdVisionInputMode =
                    ...
                    birdVisionWindow.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
                }
            }
        ...
        super.setWebChromeClient(object : WebChromeClient() {
            ...
            override fun onShowFileChooser(
                ...
                birdVisionFileChooserHandler?.invoke(filePathCallback)
                return true
            }
            ...
            override fun onCreateWindow(
                ...
                birdVisionHandleCreateWebWindowRequest(resultMsg)
                return true
            }
        })
    }
    ...
    private fun birdVisionEnableStableInputScroll() {
        evaluateJavascript(
            """
        (function() {
    ...
    fun birdVisionFLoad(link: String) {
        super.loadUrl(link)
    }

    private fun birdVisionHandleCreateWebWindowRequest(resultMsg: Message?) {
        if (resultMsg == null) return
        if (resultMsg.obj != null && resultMsg.obj is WebView.WebViewTransport) {
            val transport = resultMsg.obj as WebView.WebViewTransport
            val windowWebView =
                BirdVisionVi(birdVisionContext, birdVisionCallback, birdVisionWindow)
            transport.webView = windowWebView
            resultMsg.sendToTarget()
            birdVisionCallback.birdVisionHandleCreateWebWindowRequest(windowWebView)
        }
    }
